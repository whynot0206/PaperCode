...
# 从各个子模块导入核心类、函数和工具
# 通过__all__列表明确导出哪些内容，控制公开接口
# 提供了多个映射字典，方便通过字符串名称动态选择组件
# preprocess中 tokenizer = str2tokenizer[args.tokenizer](args) 例如：如果args.tokenizer="bert"，则相当于调用 BertTokenizer(args)
str2tokenizer = {"char": CharTokenizer, "space": SpaceTokenizer, "bert": BertTokenizer, "bertflow": BertTokenizer}
str2dataset = {"bert": BertDataset,"bertflow": BertFlowDataset, "lm": LmDataset, "mlm": MlmDataset,
               "bilm": BilmDataset, "albert": AlbertDataset, "seq2seq": Seq2seqDataset,
               "t5": T5Dataset, "cls": ClsDataset, "prefixlm": PrefixlmDataset}
str2dataloader = {"bert": BertDataLoader, "bertflow": BertFlowDataLoader, "lm": LmDataLoader, "mlm": MlmDataLoader,
