...
#https://github.com/SimiaoZuo/MoEBERT/blob/cd21f571ae25e4483c77d8b5bae1d0803deb5a89/src/transformers/moebert/moe_layer.py
...
class MoELayer(nn.Module):
    ...
    def _forward_proto(self, x, input_ids,proto):
        bsz, seq_len, dim = x.size()
    ...
    def forward(self, x, input_ids, attention_mask,proto=None):
        if self.route_method == "gate-token":
            ...
            bsz, seq_len, dim = x.size()
            #print(proto)
            proto = proto.repeat(seq_len,1).transpose(0,1).contiguous()
            #print(proto.shape)
            x, balance_loss, gate_load = self._forward_proto(x, input_ids, proto)
            #print(gate_load)
        else:
            raise KeyError("Routing method not supported.")
